<?php $page="14";  $ver="1";  $lang="pl";  $tree=":0:100:10:";  $INCLUDE_PATH="include";  $MAIN_INCLUDE_PATH="include";  $PAGE_PATH="php";  $IMAGES="images/1";  $UIMAGES="images";  $UFILES="att";  $SERVER_ID="11";  $prevpage="promienko/akcje/";  $pagetype=1; include("include/pre.php");?><?php include("include/action.php");?>
...
<head>
	...
	<meta name="WebKameleonId" content="server=11; page=14; ver=1; lang=pl" />
...
<body    ><div class="body_h">
  ...
  <div class="body_menu">
  	<div class="szukaj"><?php parse_str("more=link.php&cos=&next=link.php&size=&class=&costxt=&title=&pagetype=1&self=link.php&tree=:0:100:10:&sid=4226&prevpage=promienko%2Fakcje%2F&INCLUDE_PATH=include&html=js.php"); 
...
<li class="active"><a href="promienko/akcje/">Akcje</a></li>
...
<li ><a href="promienko/pobor-energii/">Pobór energii</a></li>
  ...
  <div class="title"><h2>Akcja <img src=images/1/szablon/dot.png></h2></div>
  <div  class="txt box_szary"><form method="post" action="promienko/akcje/" id="kameleon_form_sid_4228" name="kameleon_form_1">
    <p><input type="hidden" name="action" value="inputoutput" /> {sql_table_form}
    <table class="tabela_1" border="1" cellspacing="0" cellpadding="4">
        <tbody>
            <tr>
                <td>Master wejście:</td>
                <td><select id="we_master" onchange="selectWeOnChange(this,0)"></select> <input id="_we_master" type="hidden" name="actions[a_input_master]" value="{a_input_master}" /></td>
            </tr>
            <tr>
                <td>Moduł wejście:</td>
                <td><select id="we_module" onchange="selectWeOnChange(this,1);createCheckBox('we_input','a_input_adr','{a_input_adr}')"></select> <input id="_we_module" type="hidden" name="actions[a_input_module]" value="{a_input_module}" /></td>
            </tr>
            <tr>
                <td>Adresy wejść w obrębie modułu:<select style="display: none" id="we_input"></select></td>
                <td id="a_input_adr">&nbsp;</td>
            </tr>
            <tr>
                <td>Master wyjście:</td>
                <td><select id="wy_master" onchange="selectWyOnChange(this,0)"></select> <input id="_wy_master" type="hidden" name="actions[a_output_master]" value="{a_output_master}" /></td>
            </tr>
            <tr>
                <td>Moduł wyjście:</td>
                <td><select id="wy_module" onchange="selectWyOnChange(this,1)"></select> <input id="_wy_module" type="hidden" name="actions[a_output_module]" value="{a_output_module}" /></td>
            </tr>
            <tr>
                <td>Adres wyjście:</td>
                <td><select id="wy_output" onchange="selectWyOnChange(this,2)"></select> <input id="_wy_output" type="hidden" name="actions[a_output_adr]" value="{a_output_adr}" /></td>
            </tr>
            <tr>
                <td>Usunąć poprzednie:</td>
                <td><input checked="checked" type="checkbox" name="deletebefore" value="1" /></td>
            </tr>
        </tbody>
    </table>
    </p>
    <p><input style="display: none" id="zapisz" class="button" type="submit" value="Połącz wejście z wyjściem" /></p>
</form>
...
setTimeout(ustawZaChwile,500);
</script><?php parse_str("more=link.php&cos=&next=promienko%2Fakcje%2F&size=&class=box_szary&costxt=base64%&title=Akcja&pagetype=1&self=link.php&tree=:0:100:10:&sid=4228&prevpage=promienko%2Fakcje%2F&INCLUDE_PATH=include&html=eval/eval.php"); 
 include("include/eval/eval.php");  ?>
  ...
  <div  class="txt"><?php parse_str("more=link.php&cos=&next=link.php&size=&class=&costxt=&title=&pagetype=1&self=link.php&tree=:0:100:10:&sid=4229&prevpage=promienko%2Fakcje%2F&INCLUDE_PATH=include&html=wewy-select.php"); 
