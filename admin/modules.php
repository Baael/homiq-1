<?php $page="2";  $ver="1";  $lang="pl";  $tree=":0:100:";  $INCLUDE_PATH="include";  $MAIN_INCLUDE_PATH="include";  $PAGE_PATH="php";  $IMAGES="images/1";  $UIMAGES="images";  $UFILES="att";  $SERVER_ID="11";  $prevpage="promienko/";  $pagetype=1; include("include/pre.php");?><?php include("include/action.php");?>
...
<head>
	...
	<meta name="WebKameleonId" content="server=11; page=2; ver=1; lang=pl" />

	<meta name="ftpdate" content="Fri, 04 Nov 2011 20:28:21 +0100" />
...
<body    ><div class="body_h">
  ...
  <div class="body_menu">
  	<div class="szukaj"><?php parse_str("more=modules.php&cos=&next=modules.php&size=&class=&costxt=&title=&pagetype=1&self=modules.php&tree=:0:100:&sid=4226&prevpage=promienko%2F&INCLUDE_PATH=include&html=js.php"); 
...
<li class="active"><a href="modules.php">Moduły</a></li>
...
<li ><a href="promienko/pobor-energii/">Pobór energii</a></li>
  ...
  <div class="title"><h2>Moduły <img src=images/1/szablon/dot.png></h2></div>
  <div  class="txt box_szary"><p>{loop:navi}Pozycji: <strong>{ile}</strong>, strony: <a href="javascript:sql_table_navi_{sid}({first})">pierwsza</a> {loop:strony} <a class="navi{active}" href="javascript:sql_table_navi_{sid}({strona})">{text}</a> {endloop:strony} <a href="javascript:sql_table_navi_{sid}({last})">ostatnia</a>{endloop:navi}</p>
<table class="tabela_1" border="1" cellspacing="0" cellpadding="4">
    <thead>
        <tr>
            <th id="m_id" width="5%">Lp</th>
            <th id="m_name">Nazwa</th>
            <th id="m_symbol">Symbol</th>
            <th id="m_master">Master</th>
            <th id="m_adr">Adres</th>
<th id="m_serial">Serial</th>
            <th id="m_type" width="3%">T</th>
            <th id="m_state" width="3%">S</th>
            <th>Akcje</th>
            <th>Roleta</th>
        </tr>
    </thead>
    <tbody>
        <!-- loop:sql_table --><!-- if:m_active -->
        <tr>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{lp}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_name}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_symbol}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_master}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_adr}</a></td>
<td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_serial}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_type}</a></td>
            <td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_state}</a></td>
            <td>{if:m_serial}<a href="{self}{sign}action=ID.0&amp;m_id={m_id}&amp;{sql_table_link}"><img border="0" alt="" src="images/id.gif" /></a>{endif:m_serial}{if:!m_serial} <a href="{self}{sign}action=GS&amp;m_id={m_id}&amp;{sql_table_link}"><img border="0" alt="" src="images/gs.gif" /></a> {endif:!m_serial} <a onClick="return confirm('na pewno?')" href="{self}{sign}action=delete&amp;table=modules,m_id&amp;m_id={m_id}&amp;{sql_table_link}"><img border="0" alt="Usuń" src="images/trash.gif" /></a></td>
            <td>{if:m_type=R}<div>
<script language="javascript">
ajax_roleta('{m_id}','{m_state}');
</script></div>{endif:m_type=R}&nbsp;</td>
        </tr>
        <!-- endif:m_active --><!-- if:!m_active -->
...
<td><a href="{next}{sign}m_id={m_id}&amp;{sql_table_link}">{m_serial}</a></td>
        ...
        <!-- endif:!m_active --><!-- endloop:sql_table -->
    </tbody>
</table>
<p>&nbsp;</p>
<p><a href="{next}{sign}m_active=1">Dodaj</a></p><?php parse_str("more=modules.php&cos=&next=module.php&size=30&class=box_szary&costxt=YTo3OntzOjM6InNpZCI7czo0OiI0MjA2IjtzOjQ6ImV2YWwiO3M6MTAzOiJpZiAoIXN0cmxlbigk%0D%0AX1JFUVVFU1Rbb3JkZXJieV0pKSANCnsNCiAgICRfUkVRVUVTVFtvcmRlcmJ5XT0nbV9uYW1lJzsN%0D%0ACiAgICRfUkVRVUVTVFtvcmRlcmhvd109J0FTQyc7DQp9IjtzOjM6InNxbCI7czoxMDM6IlNFTEVD%0D%0AVCAqIEZST00gbW9kdWxlcw0KJHNxbF9maWx0cl93aGVyZSAkc3FsX2ZpbHRyIA0KT1JERVIgQlkg%0D%0AJF9SRVFVRVNUW29yZGVyYnldICRfUkVRVUVTVFtvcmRlcmhvd10gDQoiO3M6NToiY291bnQiO3M6%0D%0ANjQ6IlNFTEVDVCBjb3VudChtX2lkKSBGUk9NIG1vZHVsZXMgDQokc3FsX2ZpbHRyX3doZXJlICRz%0D%0AcWxfZmlsdHIgDQoiO3M6OToibG9vcF9ldmFsIjtzOjA6IiI7czo2OiJoZWFkZXIiO3M6MToiMSI7%0D%0Aczo2OiJmaWx0cnkiO3M6MDoiIjt9&title=Modu%C5%82y&pagetype=1&self=modules.php&tree=:0:100:&sid=4206&prevpage=promienko%2F&INCLUDE_PATH=include&html=sql_table/sql_table.php"); 
