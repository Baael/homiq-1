<?php $page="4";  $ver="1";  $lang="pl";  $tree=":0:100:";  $INCLUDE_PATH="include";  $MAIN_INCLUDE_PATH="include";  $PAGE_PATH="php";  $IMAGES="images/1";  $UIMAGES="images";  $UFILES="att";  $SERVER_ID="11";  $prevpage="promienko/";  $pagetype=1; include("include/pre.php");?><?php include("include/action.php");?>
...
<head>
	...
	<meta name="WebKameleonId" content="server=11; page=4; ver=1; lang=pl" />
...
<body    ><div class="body_h">
  ...
  <div class="body_menu">
  	<div class="szukaj"><?php parse_str("more=outputs.php&cos=&next=outputs.php&size=&class=&costxt=&title=&pagetype=1&self=outputs.php&tree=:0:100:&sid=4226&prevpage=promienko%2F&INCLUDE_PATH=include&html=js.php"); 
...
<li class="active"><a href="outputs.php">Wyjścia</a></li>
...
<li ><a href="promienko/pobor-energii/">Pobór energii</a></li>
  ...
  <div class="title"><h2>Wyjścia <img src=images/1/szablon/dot.png></h2></div>
  <div  class="txt"><p>{loop:navi}Pozycji: <strong>{ile}</strong>, strony: <a href="javascript:sql_table_navi_{sid}({first})">pierwsza</a> {loop:strony} <a class="navi{active}" href="javascript:sql_table_navi_{sid}({strona})">{text}</a> {endloop:strony} <a href="javascript:sql_table_navi_{sid}({last})">ostatnia</a>{endloop:navi}</p>
<table border="1" cellpadding="4" cellspacing="0" class="tabela_1">
	...
	<tbody>
<!-- loop:sql_table --><!-- if:o_active -->		<tr>
			<td>
				<a href="{next}{sign}o_id={o_id}&amp;{sql_table_link}">{lp}</a></td>
			<td>
				<a href="{next}{sign}o_id={o_id}&amp;{sql_table_link}">{o_name}</a></td>
			<td>
				<a href="{next}{sign}o_id={o_id}&amp;{sql_table_link}">{o_master}-<strong>{o_module}</strong>-{o_adr}</a></td>
			<td>
				{c} {if:o_sleep}{if:c=2}<a href="{self}{sign}action=refresh&amp;o_id={o_id}&amp;{sql_table_link}"><img align="absMiddle" alt="" border="0" src="images/refresh.gif" /></a>{endif:c=2}{if:c=4}<a href="{self}{sign}action=refresh&amp;o_id={o_id}&amp;{sql_table_link}"><img align="absMiddle" alt="" border="0" src="images/refresh.gif" /></a>{endif:c=4}{endif:o_sleep} {if:t}(za {t}){endif:t}</td>
			<td>
				<a href="{self}{sign}action=O&amp;o_id={o_id}&amp;{sql_table_link}"><img alt="" border="0" height="40" src="images/{o_type}-{o_state}.gif" width="30" /></a></td>
			<td>
				<a href="{self}{sign}action=delete&amp;table=outputs,o_id&amp;o_id={o_id}&amp;{sql_table_link}" onclick="return confirm('na pewno?')"><img alt="Usuń" border="0" src="images/trash.gif" /></a></td>
		</tr>
<!-- endif:o_active --><!-- if:!o_active -->		<tr disabled="disabled">
			<td>
				<a href="{next}{sign}o_id={o_id}&amp;{sql_table_link}">{lp}</a></td>
			<td>
				<a href="{next}{sign}o_id={o_id}&amp;{sql_table_link}">{o_name}</a></td>
...
<p>&nbsp;</p>
<p><a href="{next}?o_active=1">Dodaj</a></p><?php parse_str("more=outputs.php&cos=&next=output.php&size=60&class=&costxt=YTo3OntzOjM6InNpZCI7czo0OiI0MjExIjtzOjQ6ImV2YWwiO3M6MTI1OiJpZiAoIXN0cmxlbigk%0D%0AX1JFUVVFU1Rbb3JkZXJieV0pKSANCnsNCiAgICRfUkVRVUVTVFtvcmRlcmJ5XT0nb19tb2R1bGUs%0D%0Ab19hZHInOw0KICAgJF9SRVFVRVNUW29yZGVyaG93XT0nQVNDJzsNCn0NCg0KJHQ9dGltZSgpOyI7%0D%0AczozOiJzcWwiO3M6MTAzOiJTRUxFQ1QgKiBGUk9NIG91dHB1dHMNCiRzcWxfZmlsdHJfd2hlcmUg%0D%0AJHNxbF9maWx0ciANCk9SREVSIEJZICRfUkVRVUVTVFtvcmRlcmJ5XSAkX1JFUVVFU1Rbb3JkZXJo%0D%0Ab3ddIA0KIjtzOjU6ImNvdW50IjtzOjY0OiJTRUxFQ1QgY291bnQob19pZCkgRlJPTSBvdXRwdXRz%0D%0AIA0KJHNxbF9maWx0cl93aGVyZSAkc3FsX2ZpbHRyIA0KIjtzOjk6Imxvb3BfZXZhbCI7czo0NjY6%0D%0AIiRzcWw9IlNFTEVDVCBjb3VudCgqKSBBUyBjIEZST00gYWN0aW9uIFdIRVJFIGFfb3V0cHV0X21h%0D%0Ac3Rlcj0nJHRhYltvX21hc3Rlcl0nIEFORCBhX291dHB1dF9tb2R1bGU9JyR0YWJbb19tb2R1bGVd%0D%0AJyBBTkQgYV9vdXRwdXRfYWRyPSckdGFiW29fYWRyXSciOw0KcGFyc2Vfc3RyKHF1ZXJ5MnVybCgk%0D%0Ac3FsKSk7DQokdGFiW2NdPSRjOw0KDQoka2llZHk9MDsNCiRzcWw9IlNFTEVDVCBzX3F1ZXVlLSR0%0D%0AIEFTIGtpZWR5IEZST00gc2VuZCBXSEVSRSBzX21hc3Rlcj0nJHRhYltvX21hc3Rlcl0nIA0KICAg%0D%0AICAgICAgQU5EIHNfZHN0PSckdGFiW29fbW9kdWxlXScgQU5EIHNfY21kPSdPLiR0YWJbb19hZHJd%0D%0AJw0KICAgICAgICAgQU5EIHNfcXVldWU%2BJHQiOw0KDQpwYXJzZV9zdHIocXVlcnkydXJsKCRzcWwp%0D%0AKTsNCmlmICgka2llZHkpICRraWVkeT1kYXRlKCdIOmk6cycsJGtpZWR5LTM2MDApOw0KJHRhYlt0%0D%0AXT0ka2llZHk7DQoiO3M6NjoiaGVhZGVyIjtzOjE6IjEiO3M6NjoiZmlsdHJ5IjtzOjA6IiI7fQ%3D%3D&title=Wyj%C5%9Bcia&pagetype=1&self=outputs.php&tree=:0:100:&sid=4211&prevpage=promienko%2F&INCLUDE_PATH=include&html=sql_table/sql_table.php"); 
